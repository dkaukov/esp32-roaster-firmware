<template>
  <header class="rounded-[1.75rem] bg-white px-6 py-3 shadow-[0_0_50px_rgba(232,232,232,0.9)] md:px-8">
    <div class="flex items-center justify-between gap-6">
      ...
      <nav class="hidden items-center gap-3 md:flex" aria-label="main navigation">
        <router-link
          ...
          :key="item.to"
          :to="item.to"
          class="rounded-2xl px-6 py-2.5 text-[0.95rem] font-semibold no-underline transition hover:no-underline"
    ...
    <nav v-if="open" class="mt-4 grid gap-2 md:hidden" aria-label="mobile navigation">
      <router-link
        v-for="item in items"
        :key="`mobile-${item.to}`"
        :to="item.to"
        class="rounded-2xl px-4 py-2.5 text-base font-semibold no-underline transition hover:no-underline"
        :class="itemClasses(item.to)"
        @click="open = false"
      >
        {{ item.label }}
...
export default {
  ...
  methods: {
    itemClasses(path) {
      ...
      return isActive
        ? "bg-[#2f3138] text-white hover:bg-[#3a3d46] hover:text-white focus:bg-[#3a3d46] focus:text-white focus:outline-none focus-visible:bg-[#3a3d46] focus-visible:text-white focus-visible:outline-none"
        : "text-slate-500 hover:bg-slate-50 hover:text-slate-700 focus:bg-slate-50 focus:text-slate-700 focus:outline-none focus-visible:bg-slate-50 focus-visible:text-slate-700 focus-visible:outline-none";
    },
  },
};
</script>
